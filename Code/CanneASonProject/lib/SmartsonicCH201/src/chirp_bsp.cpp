...
int CHIRP_PIN_PROG[2] = {PROG1, PROG2};
int CHIRP_PIN_INT[2] = {INT1, INT2};
...
void chbsp_board_init(ch_group_t *grp_ptr){
    ...
    int tempPinProg, tempPinInt;

    for (uint8_t dev_num = 0; dev_num < ch_get_num_ports(grp_ptr); dev_num++) {
        // Get pin specification for individual sensor
        tempPinProg = CHIRP_PIN_PROG[dev_num];
        tempPinInt = CHIRP_PIN_INT[dev_num];

        pinMode(tempPinProg, OUTPUT);
        pinMode(tempPinInt, OUTPUT);

        // Initialize every programm pin as low, to assert them later individually
        digitalWrite(tempPinProg, LOW);
    }

 	for (uint8_t dev_num = 0; dev_num < ch_get_num_ports(grp_ptr); dev_num++) {
        //Reset bytes to verify if sensor is respondable
        sig_bytes[0] = 0; sig_bytes[1] = 0;
        tempPinProg = CHIRP_PIN_PROG[dev_num];

 		// Assert program line for individual sensor, so it's respondable to its default I2C adress
        digitalWrite(tempPinProg, HIGH);

        // Move memory pointer to 0x00 and start request
        Wire.beginTransmission(CH_I2C_ADDR_PROG);
        Wire.write(0x00);
        Wire.endTransmission(false);
        Wire.requestFrom(CH_I2C_ADDR_PROG, 2);    // request 2 bytes from Prog Address
        sig_bytes[0] = Wire.read();
        sig_bytes[1] = Wire.read();
        Serial.print("Device "); Serial.print(dev_num);
...
void chbsp_program_enable(ch_dev_t *dev_ptr){
    digitalWrite(CHIRP_PIN_PROG[dev_ptr->io_index], HIGH);
}
...
void chbsp_program_disable(ch_dev_t *dev_ptr){
    digitalWrite(CHIRP_PIN_PROG[dev_ptr->io_index], LOW);
}
...
void chbsp_group_set_int1_dir_out(ch_group_t *grp_ptr){
    ...
    for (uint8_t dev_num = 0; dev_num < ch_get_num_ports(grp_ptr); dev_num++) {
        dev_ptr = ch_get_dev_ptr(grp_ptr, dev_num);
        if(ch_sensor_is_connected(dev_ptr))
            pinMode(CHIRP_PIN_INT[dev_num], OUTPUT);
 	}
}
...
void chbsp_set_int1_dir_out(ch_dev_t *dev_ptr){
    pinMode(CHIRP_PIN_INT[dev_ptr->io_index], OUTPUT);
}
...
void chbsp_group_set_int1_dir_in(ch_group_t *grp_ptr){
    ...
    for (uint8_t dev_num = 0; dev_num < ch_get_num_ports(grp_ptr); dev_num++) {
        ...
        if(ch_sensor_is_connected(dev_ptr))
            pinMode(CHIRP_PIN_INT[dev_num], OUTPUT);
            digitalWrite(CHIRP_PIN_INT[dev_num], LOW);
            pinMode(CHIRP_PIN_INT[dev_num], INPUT);
 	}
}
...
void chbsp_set_int1_dir_in(ch_dev_t *dev_ptr){
    pinMode(CHIRP_PIN_INT[dev_ptr->io_index], INPUT);
...
void chbsp_int1_clear(ch_dev_t *dev_ptr){
    digitalWrite(CHIRP_PIN_INT[dev_ptr->io_index], LOW);
}
...
void chbsp_group_int1_set(ch_group_t *grp_ptr){
    for (uint8_t dev_num = 0; dev_num < ch_get_num_ports(grp_ptr); dev_num++) {
        digitalWrite(CHIRP_PIN_INT[dev_num], HIGH);
    }
}
...
void chbsp_int1_set(ch_dev_t *dev_ptr){
    digitalWrite(CHIRP_PIN_INT[dev_ptr->io_index], HIGH);
}


ch_io_int_callback_t globalCallbackFunction = nullptr;
//void IRAM_ATTR ISR(){
//    ch_io_int_callback_t callbackFunc = *io_int_callback_ptr;
//    callbackFunc(grp_ptr);
//    find reponsible pin
//    for(alle pins durchgehen){
//        welcher ist Schuld
//        if(this pin ist schuld){
//            globalCallbackFunction(groupPtr, ioIndex);
//        }
//    }
//}
...
void chbsp_group_int1_interrupt_enable(ch_group_t *grp_ptr){
    for (uint8_t dev_num = 0; dev_num < ch_get_num_ports(grp_ptr); dev_num++) {
        attachInterrupt(digitalPinToInterrupt(CHIRP_PIN_INT[dev_num]), grp_ptr->io_int_callback, RISING);
    }
}
...
void chbsp_int1_interrupt_enable(ch_dev_t *dev_ptr){
    attachInterrupt(digitalPinToInterrupt(CHIRP_PIN_INT[dev_ptr->io_index]), dev_ptr->group->io_int_callback, RISING);
}
...
void chbsp_group_int1_interrupt_disable(ch_group_t *grp_ptr){
    for (uint8_t dev_num = 0; dev_num < ch_get_num_ports(grp_ptr); dev_num++) {
        detachInterrupt(digitalPinToInterrupt(CHIRP_PIN_INT[dev_num]));
    }
}
...
void chbsp_int1_interrupt_disable(ch_dev_t *dev_ptr){
    detachInterrupt(digitalPinToInterrupt(CHIRP_PIN_INT[dev_ptr->io_index]));
}
...
int chbsp_i2c_init(void){
    Wire.begin(PIN_SDA, PIN_SCL, UC_FREQUENCY);
    return 0;
}
...
uint8_t chbsp_i2c_get_info(ch_group_t *grp_ptr, uint8_t dev_num, ch_i2c_info_t *info_ptr){
	...
	if (dev_num <= CHBSP_MAX_DEVICES) {
		info_ptr->address = chirp_i2c_addrs[dev_num];
		info_ptr->bus_num = chirp_i2c_buses[dev_num];
...
int chbsp_i2c_write(ch_dev_t *dev_ptr, uint8_t *data, uint16_t num_bytes){
    ...
    {
        res = Wire.write(data[i]);
        if(!res){
            info |= Wire.endTransmission();
            Wire.beginTransmission(ch_get_i2c_address(dev_ptr));
            Wire.write(data[i]);
        }
    }
...
//    Serial.print(" this many bytes: "); Serial.print(num_bytes); Serial.print(" to adress: 0x"); Serial.print(ch_get_i2c_address(dev_ptr), HEX);
//    for(uint16_t i = 0; i < num_bytes; i++)
//    {
//        Serial.print(", data["); Serial.print(i); Serial.print("]: 0x"); Serial.print(data[i], HEX);
//        if(i > 100)
//            break;
//    }
    ...
    switch(info)
    {
        case 0: /*Serial.println (" -> Succesfull");*/ return 0;
        case 1: /*Serial.println (" -> data too long to fit in transmit buffer");*/ return 1;
        case 2: /*Serial.println (" -> received NACK on transmit of address.");*/ return 1;
        case 3: /*Serial.println (" -> received NACK on transmit of data.");*/ return 1;
        case 4: /*Serial.println (" -> other error");*/ return 1;
    }
}
...
//    Serial.print(" this many bytes: "); Serial.print(num_bytes); Serial.print(" to adress: 0x"); Serial.print(ch_get_i2c_address(dev_ptr), HEX);
//    Serial.print(", at mem adress: 0x"); Serial.print(highByte(mem_addr), HEX); Serial.print(lowByte(mem_addr), HEX);
//    for(uint16_t i = 0; i < num_bytes; i++)
//    {
//        Serial.print(", data["); Serial.print(i); Serial.print("]: 0x"); Serial.print(data[i], HEX);
//        if(i > 100)
...
int chbsp_i2c_read(ch_dev_t *dev_ptr, uint8_t *data, uint16_t num_bytes){
    uint8_t device_ic2_address = ch_get_i2c_address(dev_ptr);
//    Serial.print(" this many bytes: "); Serial.print(num_bytes);Serial.print(" from address: 0x"); Serial.print(device_ic2_address, HEX);
    int receivedBytes = Wire.requestFrom(device_ic2_address,num_bytes);
//    Serial.print(" | Response: received "); Serial.print(receivedBytes); Serial.print(" bytes: ");
    if(receivedBytes != num_bytes)
    {
//        Serial.println("did not receive enough bytes.");
        return 1;
    }
...
int chbsp_i2c_mem_read(ch_dev_t *dev_ptr, uint16_t mem_addr, uint8_t *data, uint16_t num_bytes){
    ...
    Wire.beginTransmission(device_ic2_address);
    uint8_t mem_addr_highByte = highByte(mem_addr);
    uint8_t mem_addr_lowByte = lowByte(mem_addr);
//    if(mem_addr_highByte > 0)
//        Wire.write(mem_addr_highByte);
//    if(mem_addr_lowByte > 0)
        Wire.write(mem_addr_lowByte);
//    Serial.print(" this many bytes: "); Serial.print(num_bytes);Serial.print(" from adress: 0x"); Serial.print(device_ic2_address, HEX);
//    Serial.print(", at mem adress: 0x"); Serial.print(mem_addr, HEX);
//    Serial.print(" transmission");
    Wire.endTransmission(false);
//    switch(Wire.endTransmission(false))
//    {
//        case 0:Serial.print(" -> succesfull"); break;
//        case 1:Serial.print (" -> data too long to fit in transmit buffer"); Serial.println(); return 1;
//        case 2:Serial.print (" -> received NACK on transmit of address."); Serial.println(); return 1;
//        case 3:Serial.print (" -> received NACK on transmit of data."); Serial.println(); return 1;
//        case 4:Serial.print (" -> other error"); return 1;
//    }

    int receivedBytes = Wire.requestFrom(device_ic2_address,num_bytes);
//    Serial.print(" | Response: received "); Serial.print(receivedBytes);Serial.print("/");Serial.print(num_bytes); Serial.print(" data: ");
    if(receivedBytes != num_bytes)
    {
//        Serial.println("did not receive enough bytes.");
        return 1;
...
//    timerAttachInterrupt(My_timer, callback_func_ptr, true);
//    timerAlarmWrite(My_timer, interval_ms*1000, true);
}
