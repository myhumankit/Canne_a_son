/*
 * Modification de la bibliothèque du M5Atom pour la faire fonctionner avec la canne
 */
...
MPU6886_custom::MPU6886_custom(){

}

void MPU6886_custom::I2C_Read_NBytes(uint8_t driver_Addr, uint8_t start_Addr, uint8_t number_Bytes, uint8_t *read_Buffer){
...
void MPU6886_custom::I2C_Write_NBytes(uint8_t driver_Addr, uint8_t start_Addr, uint8_t number_Bytes, uint8_t *write_Buffer){
...
void MPU6886_custom::getAccelAdc(int16_t* ax, int16_t* ay, int16_t* az){
...
void MPU6886_custom::getGyroAdc(int16_t* gx, int16_t* gy, int16_t* gz){
...
void MPU6886_custom::getTempAdc(int16_t *t){
...
void MPU6886_custom::getAhrsData(float *pitch,float *roll,float *yaw){
...
void MPU6886_custom::getAttitude(double *pitch, double *roll)
{
    float accX = 0;
    float accY = 0;
    float accZ = 0;
...
void MPU6886_custom::SetGyroFsr(Gscale scale)
{
...
void MPU6886_custom::SetAccelFsr(Ascale scale)
{
    unsigned char regdata;	
    regdata = (scale<<3);
    I2C_Write_NBytes(MPU6886_ADDRESS, MPU6886_ACCEL_CONFIG, 1, &regdata);
...
void MPU6886_custom::getAccelData(float* ax, float* ay, float* az){
...
void MPU6886_custom::getGyroData(float* gx, float* gy, float* gz){
  int16_t gyroX = 0;
  int16_t gyroY = 0;
  int16_t gyroZ = 0;
...
void MPU6886_custom::getTempData(float *t){
