...
typedef struct raw_output_data{
    ...
    volatile qi_t IQdata[IQ_SAMPLES_MAX]; //IQdata (see iq_output_format to decode).
}raw_output_data_t;
...
typedef struct measurement{ // size: 172 bytes
    ...
    //volatile dsp_inst_t dsp_inst[8];
...
typedef struct otp_copy{
    ...
    uint8_t product_variant; //e.g. ICU-20201, ICU-10201, etc
...
#define EVENT_GPIOPULSE       (64)
#define EVENT_COPY_OTP_TO_RAM (128)
#define EVENT_OTP_WR_STEP     (256)
